...
- Shell completion:
  - Bash: `eval "$(_SMART_TRAIN_COMPLETE=bash_source smartrain)"`
  - Zsh: `eval "$(_SMART_TRAIN_COMPLETE=zsh_source smartrain)"`
  - Fish: `eval (env _SMART_TRAIN_COMPLETE=fish_source smartrain)`
  - Для постоянной активации добавьте соответствующую команду в профиль shell (`~/.bashrc`, `~/.zshrc`, `~/.config/fish/config.fish`).

## Интерактивный UX

- В интерактивных командах ввод начинается с пустого поля: default не нужно стирать вручную.
- Если нажать Enter, принимается значение по умолчанию и оно явно отображается в строке prompt (как будто введено вручную).
- Для вопросов `yes/no` используется единый формат `... [Y/n]:` или `... [y/N]:`; Enter подставляет соответствующий default.
- Для параметров выбора по возможности используется список вариантов (можно выбирать по номеру или значению).
- После интерактивного ввода команда печатает готовый non-interactive вызов:
  - перед выполнением;
  - повторно после выполнения.
- Этот вызов можно сразу использовать для повторного запуска без интерактивного режима.
...
| `augment` | Офлайн-аугментация (`flip`/`center_rotate`/`conveyor`/`bbox_copy`, soft-balance) в новый `datasets/<name>` (`<dataset>_aug[_N]`) |
