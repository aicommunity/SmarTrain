...
| `analyze` subcommands | Typer subcommands → `_invoke_module_main("...analyze_entry", [...])` | `workflows/analyze/analyze_entry.py` → `results_analyzer.py` (facade) | `services/analyze/*` | Metrics / unified: `orchestrators/unified_gateway.py` |
...
- Unified read/write and gateway: `orchestrators/unified_gateway.py`, `smartrain/unified/` (`domain/`, `io/`).
