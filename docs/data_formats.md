...
При повторном запуске `datasets_json_former.py` существующий `datasets_info.json` читается и для каждого датасета, который снова найден на диске, в новую запись **переносятся** поля `roi_auto` и `tags` из старого файла. Остальные поля берутся из свежего скана.
