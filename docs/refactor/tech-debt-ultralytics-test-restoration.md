...
| TD-005 | DONE | `model_test_runner.run_model_test_after_setup` | Plan §3.1 alt: MPL before PT if entry bypasses CLI. | `ensure_matplotlib_training_runtime` already called at start of runner (non_interactive from args). |
...
| TD-007 | DONE | `model_test_runner` + `cli_replay` | `smartrain test --run … --formats …` from TTY still prompted for backends; replay lacked `--non-interactive`. | Skip backend/artifact prompts when `--formats` is present in argv; append `--nit` to printed `smartrain test` replay when missing. |
