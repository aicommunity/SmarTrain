...
- [`13-project-current-state.md`](./13-project-current-state.md) — актуальный narrative-snapshot архитектуры, слоёв и operational limits (дата среза в шапке файла).
...
- 2026-05-05: Дожат residual-хвост фазы D: mismatch между capability policy и runtime backend в `inference_service` теперь fail-fast (ошибка + abort), при этом добавлена alias-совместимость для допустимых runtime-идентификаторов (`ultralytics:engine/trt/onnx`) и regression-тест на hard-fail для недопустимого backend id.
...
- [x] **5-E2** Classification/segmentation readiness + consumer wiring (PR 5.4–5.5). Notes: завершён end-to-end task-aware wiring для local/external inference, включая degraded-contract для external provider payloads (empty-but-valid `classification` / `segments`) и regression coverage.
...
- 2026-05-05: Закрыт residual-хвост 5-E2 по external parity: в `inference_service` добавлена task-aware нормализация degraded payload (`classification -> {}`, `segmentation -> []`, `detection -> []`) для кейсов, где provider runtime не возвращает `probs/masks`; добавлены regression тесты `test_inference_external_provider_classification_empty_payload_normalized` и `test_inference_external_provider_segmentation_empty_payload_normalized`.
