...
- 2026-05-16: Layer-boundary continuation **closed**. Train/test execution in `services/training` and `services/testing/backends`; workflow facades for analyze, test, and datasets. LB-C7 + reporting export + `workflow_dispatch`. MTM thinned: `train_cli_parsers.py`, `train_resume_cli_service.py` (TD-LB-012 partial). Register: [`tech-debt-layer-boundaries.md`](./tech-debt-layer-boundaries.md).
