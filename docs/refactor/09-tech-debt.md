...
- 2026-07-26: **P2 burn-down (execution=1, closed).** IRFS (`--strategy irfs` / preset `irfs-default`); Optimal LRP opt-in (`--compute-lrp` → `lrp_recommendations_*.json`, report D); micro/macro F-β + `confidence_policy` + inference `--confidence-objective`; SAHI `prepare-slices` recipe; queue `--max-retries` / backoff / `Waiting retry N/M`; root `Dockerfile` cu128 + `docs/getting-started/docker.md` + `scripts/ci/docker_smoke.sh`; `scripts/balance_preset_harness.py`; broke `core → services` (guardrail forbids). Out of scope remains: Seesaw/EQL/ECM, DVC/MLflow mandate, env rename.
- 2026-07-26: **Project audit + remediation backlog.** Full report: [`docs/audit/2026-07-26-project-audit.md`](../audit/2026-07-26-project-audit.md) (RU: [`docs/ru/audit/2026-07-26-project-audit.md`](../ru/audit/2026-07-26-project-audit.md)). **P0/P1 (execution=1):** registry gateway-first; remove dead `augment_*` shims + stale `smartrain/domain` guardrail; shared interactive preamble; MFEL shim dedup; refresh [`02-duplication-matrix.md`](./02-duplication-matrix.md); split `model_convert_cli` / `dataset_augment` / `dataset_balance`; unify prompts + `resolve_run_ref`; logging + coverage/ruff/mypy on `core`/`run_model_contract`/`backends`; algorithm paper citations. **P2:** see burn-down entry above (was backlog `execution=0`, now closed).
- 2026-07-17: **Legacy inventory + `smartrain update`:** full compat map in [`legacy-compat-inventory.md`](./legacy-compat-inventory.md); phased fallback removal gate in [`legacy-fallback-removal.md`](./legacy-fallback-removal.md). Workspace on-disk migration UX is `smartrain update` (layout, weights, release unify, manifests, tests, data.yaml).
- 2026-07-15: **Docs ↔ code (weights/release):** канон `models/<detect_stem>.pt` (не только `<run_name>.pt`); release R3 `models/<ds>/<run_id>/detect_*.pt` + compat R1/R2; API split `preferred_run_model_path` / `resolve_run_model` / `materialize_preferred_run_model`; `ensure_run_layout` не для release-bundle. Таблица в [`run-layout.md`](./run-layout.md). Confidence ensure выровнен на resolve+materialize когда preferred отсутствует.
...
- 2026-05-25: **model convert TRT prep:** при пропуске экспорта ONNX (уже есть публичный `*.onnx`) интерактивный пайплайн ONNX+TRT синхронизирует dedicated `*_trtprep.onnx` перед `trtexec`, если подпись ONNX совпадает. Чистый `--format onnx` не пишет `*_trtprep`; после TRT для catalog/release кэш удаляется, для runs/ сохраняется.
- 2026-07-14: **model convert + nested release:** `_guess_run_root_for_path` больше не считает release-bundle под `models/` training run (иначе ONNX уходил в `<stem>/models/` с длинным именем).
