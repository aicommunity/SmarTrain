...
- Официальное название: **CvsDclDet detection export layout** (термин SmarTrain)
  ...
  - Примечание: конвертация в CVAT 1.1 — `smartrain dataset convert --source <path> --to cvat11` (каталог или архив `.zip`/`.tar`/`.tar.gz`); результат обычно кладётся в `converted_raw_data/`.
