...
- Запуск `smartrain <команда>` без обязательных аргументов обычно выводит справку команды.
- `smartrain train` без аргументов запускает интерактивный режим (требуется TTY-терминал).
