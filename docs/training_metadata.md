...
Секция **`inference`** добавляется при успешном или запланированном вызове `val()` и содержит только явно заданные или вычисленные параметры (пустые значения в файл не пишутся).
...
- **dataset**: Информация о датасете
  ...
  - **hash**: Хеш датасета (8 символов hex), вычисленный на основе структуры папок, имен файлов и их размеров. Может быть `null` если вычисление хеша не удалось
...
Параметры валидации/теста, переданные в Ultralytics `val()` из `model_training_module.py`:

- **imgsz**: размер изображения для val (флаг `--val-imgsz`, иначе как у обучения `--img-size`)
- **conf**: порог confidence (`--val-conf`), если указан
- **iou**: порог IoU (`--val-iou`), если указан
...
Директории с результатами обучения автоматически получают имя с датой, временем начала обучения и хешем датасета:

**Формат:** `{YYYY-MM-DD_HH-MM}_{model}_{epochs}epochs-{hash}`

**Примеры:**
- `2024-12-06_13-32_yolo11n_10epochs-a1b2c3d4`
- `2024-12-06_14-15_yolov8s_50epochs-5e6f7g8h`
- `2024-12-07_09-45_yolov11m_100epochs-9i0j1k2l`

Где `{hash}` - это 8-символьный хеш датасета, вычисленный на основе структуры папок, имен файлов и их размеров. Если вычисление хеша не удалось, хеш не добавляется в имя папки.
...
- Уникально идентифицировать датасет, использованный для обучения
