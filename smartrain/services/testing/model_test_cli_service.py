...
def _prompt_export_backends_interactive(root_dir: str, candidates: dict[str, list[str]]) -> list[str]:
    ...
    surf.print_numbered_options("test backends", options)
    default_nums = [str(i + 1) for i, e in enumerate(entries) if e[2]]
    default = ",".join(default_nums) if default_nums else "1"
    raw = surf.prompt_text("Select backends to test (comma-separated numbers)", default=default).strip() or default
    out: list[str] = []
    seen: set[str] = set()
...
def _prompt_artifact_selection_interactive(candidates: dict[str, list[str]]) -> list[tuple[str, str]]:
    ...
    if not options:
        return []
    surf.print_numbered_options("models", options)
    default = ",".join(str(i) for i in mapping.keys())
    raw = surf.prompt_text("Select models for test (comma-separated numbers)", default=default).strip() or default
...
def _resolve_target(args: argparse.Namespace, layout: WorkspaceLayout) -> tuple[str, str, str, str | None]:
    ...
    if args.model_name:
        model_path, model_key = resolve_model_from_name(layout, str(args.model_name).strip())
...
def _pick_interactive_target(layout: WorkspaceLayout) -> tuple[str, str, str, str | None]:
    ...
    selected = surf.prompt_choice("Test source", options, default="runs")
    if selected == "runs":
        ...
        surf.print_numbered_options("runs", printable)
...
def _should_rerun_existing_match(
    *,
    ...
    if force:
        return True
    matches = surf.has_matching_test_artifacts(
        root_dir,
    ...
    if not matches:
        if surf.has_complete_test_artifacts(root_dir, format_name):
...
def _run_native_backend_isolated(
    *,
    ...
    try:
        ...
        proc = surf.subprocess.run(cmd, text=True)
...
def _collect_interactive_rerun_decisions(
    *,
    ...
    if "pt" in formats:
        if target_kind == "runs" and (not missing_only or not surf.has_complete_test_artifacts(root_dir, "pt")):
        ...
        elif target_kind in {"models", "weights"} and (not missing_only or not surf.has_complete_test_artifacts(root_dir, "pt")):
            decisions[_artifact_key("pt", primary_path)] = _should_rerun_existing_match(
                interactive=interactive,
                force=force,
                root_dir=root_dir,
    ...
    if selected_artifacts:
        ...
        for fmt in ("onnx", "engine", "trt"):
            if fmt not in formats:
                continue
            if missing_only and surf.has_complete_test_artifacts(root_dir, fmt):
                continue
            try:
                artifact_path = surf._resolve_existing_artifact(
                    root_dir=root_dir,
...
def main(argv: list[str] | None = None) -> None:
    ...
    request = make_command_request(
        "test", argv_list, interactive_allowed=surf.is_interactive_allowed(argv_list)
    )
