...
def _to_rel_split(root: str, raw: str) -> str:
    """Single train/val/test string relative to dataset root."""
    s = _strip_dot_slash(raw)
    if not s:
        return s
    root_abs = os.path.abspath(root)
    # Already relative to root (posix style)
...
def _foreign_absolute_to_split_relative(dataset_root: str, t: str) -> str | None:
    """
    Old exports often embed another machine's absolute paths. If this dataset root
    actually contains the expected split folders, map by path suffix (…/train/images → train/images).
    """
...
def normalize_data_yaml_mapping(dataset_root: str, data: Mapping[str, Any]) -> dict[str, Any]:
    """Return a new dict suitable for Ultralytics portable data.yaml."""
...
def iter_dataset_roots_with_data_yaml(datasets_root: str) -> list[str]:
    """
    Every directory under datasets_root that contains a data.yaml (any depth),
    excluding a data.yaml placed directly in datasets_root itself.
    """
