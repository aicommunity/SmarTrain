...
def load_target(ref: str, *, source_kind: str | None = None, options: UnifiedGatewayOptions | None = None) -> UnifiedPayload:
    opts = options or UnifiedGatewayOptions()
...
def persist_unified_snapshot(ref: str, *, source_kind: str | None = None) -> WriteReport:
    """Load unified payload for ref and write snapshot + manifest under target root."""
    payload = load_target(ref, source_kind=source_kind, options=UnifiedGatewayOptions(validate=True))
    return write_unified_snapshot(payload, ref)
...
def resolve_task_context(
    ...
    *,
    ...
    options: UnifiedGatewayOptions | None = None,
) -> UnifiedIdentity:
    ...
    if not payload.models:
        raise ValueError(f"No unified model for ref={ref!r}")
    m = payload.models[0]
    ...
    for fmt, csv_path in sorted(by_fmt.items()):
        ...
        out.append(
            UnifiedMetricsRef(
                ...
                producer="smartrain.services.analyze.metrics_reader",
                task_type=task_type,
            )
        )
    return out
...
def load_metrics(
    ...
    *,
    ...
    """
    Load test metrics CSVs for a run or model root as unified metric refs (PR 6.5).
...
def load_predictions(
    ...
    *,
    ...
    split: str | None = None,
    options: UnifiedGatewayOptions | None = None,
) -> list[UnifiedPredictionRef]:
    """
    Load available prediction artifacts as unified prediction refs (PR 6.5).
