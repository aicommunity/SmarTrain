...
from smartrain.core.runtime.run_artifacts import preferred_run_model_path, materialize_preferred_run_model, scan_run_models
...
def unified_target_from_run(run_dir: Path) -> UnifiedModelTarget:
    ...
    if not candidates:
        best = Path(preferred_run_model_path(str(run_dir), ".pt"))
        if not best.is_file():
    ...
    return UnifiedModelTarget(source_kind="runs", source_id=run_dir.name, model_path=picked)


def unified_target_from_model_dir(model_dir: Path) -> UnifiedModelTarget:
    ...
    return UnifiedModelTarget(source_kind="models", source_id=model_dir.name, model_path=picked.resolve())
