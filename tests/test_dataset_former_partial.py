...
def test_fusion_interactive_options_apply_defaults(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("smartrain.services.datasets.dataset_former._prompt_dataset_selection", lambda available: ["ds_a", "ds_b"])
    monkeypatch.setattr("sys.stdin.isatty", lambda: True)
    ...
    monkeypatch.setattr("smartrain.services.datasets.dataset_former._prompt_interactive_options", _fake_options)
