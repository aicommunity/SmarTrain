...
def test_interactive_skips_imbalance_strength_when_balancing_off(capsys) -> None:
    args = argparse.Namespace(
        dataset=None,
        classes=None,
        output_name=None,
    ...
    assert args.aug_class_aware_geo is False
    out = capsys.readouterr().out
    assert "Balancing strength" not in out
    assert "Class-aware geo augment skipped" in out
    assert "Sub-block: deduplication" in out
...
def test_interactive_prompts_tail_budget_when_cap_positive(capsys) -> None:
    args = argparse.Namespace(
        dataset=None,
        classes=None,
        output_name=None,
    ...
    assert args.aug_budget_tail_gamma == 1.5
