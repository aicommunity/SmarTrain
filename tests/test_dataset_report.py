...
def test_dataset_report_markdown_and_assets(tiny_workspace: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from smartrain.workflows.datasets.dataset_report import main
...
def test_report_odt_builtin_and_pdf_fpdf2_without_pandoc(
    ...
    monkeypatch.setattr("smartrain.workflows.datasets.dataset_report._try_pandoc_odt", lambda *a, **k: False)
    monkeypatch.setattr("smartrain.workflows.datasets.dataset_report._try_pandoc_pdf", lambda *a, **k: False)
    from smartrain.workflows.datasets.dataset_report import main
