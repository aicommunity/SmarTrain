...
def test_run_ultralytics_backend_passes_canonical_val_kwargs(
    ...
    monkeypatch.setattr("smartrain.workflows.testing.model_test_backends.YOLO", _FakeModel)
    monkeypatch.setattr(
        "smartrain.workflows.testing.model_test_backends._save_metrics_csv_for_format",
        lambda *_a, **_k: str(tmp_path / "ignored.csv"),
    )
    monkeypatch.setattr(
        "smartrain.workflows.testing.model_test_backends._finalize_ultralytics_pt_test_dir",
        lambda **_k: None,
    )
    monkeypatch.setattr(
        "smartrain.workflows.testing.model_test_backends._ensure_confidence_recommendations_for_explicit_artifact",
        lambda **_k: None,
    )
    monkeypatch.setattr(
        "smartrain.workflows.testing.model_test_backends.persist_target_test_artifacts_state",
        lambda *_a, **_k: None,
    )
    monkeypatch.setattr(
        "smartrain.workflows.testing.model_test_backends._collect_test_system_profile",
        lambda **_k: {},
    )
