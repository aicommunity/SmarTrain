...
def test_apply_migrates_legacy_run_to_canonical_snapshot(tmp_path: Path) -> None:
    ...
    snap = run_dir / ".smartrain" / "unified" / "snapshot.json"
    man = run_dir / ".smartrain" / "unified" / "manifest.json"
    assert snap.is_file()
    assert man.is_file()
...
def test_apply_continue_on_error_migrates_other_targets(tmp_path: Path, monkeypatch) -> None:
    ...
    assert (run_ok / ".smartrain" / "unified" / "snapshot.json").is_file()
    assert not (run_fail / ".smartrain" / "unified" / "snapshot.json").is_file()
...
def test_apply_model_source_kind_discovers_model_directories(tmp_path: Path) -> None:
    ...
    assert report["stats"]["migrated"] == 1
    assert (model_dir / ".smartrain" / "unified" / "snapshot.json").is_file()
